...
export const authInterceptor: HttpInterceptorFn = (
  ...
  return next(authReq).pipe(
    catchError((error: HttpErrorResponse) => {
      if (
        error.status === 401 &&
        !req.url.includes('/auth/metamask/refresh') &&
        !req.url.includes('/auth/metamask/login')
      ) {
...
function handle401Error(
  ...
  if (!isRefreshing) {
    ...
    if (refreshToken) {
      // Construction de l'URL avec le paramètre de requête en utilisant les constantes API
      const urlWithParams = `${API_CONSTANTS.GATEWAY_URL}${API_CONSTANTS.ENDPOINTS.AUTH.REFRESH}?refreshToken=${encodeURIComponent(
        refreshToken
      )}`;
