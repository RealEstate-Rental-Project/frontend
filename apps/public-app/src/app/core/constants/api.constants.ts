...
export const API_CONSTANTS = {
  ...
  ENDPOINTS: {
    ...
    ROOMS: {
      BY_PROPERTY: (propertyId: number) =>
        `/api/property-microservice/rooms/property/${propertyId}`,
    },
    ROOM_IMAGES: {
      BY_ID: (id: number) =>
        `/api/property-microservice/properties/room-images/${id}`,
      BY_ROOM: (roomId: number) =>
        `/api/property-microservice/properties/room-images/room/${roomId}`,
    },
    RENTAL_AGREEMENT: {
      BASE: '/api/rentalAgreement-microservice/rental-requests',
      BY_TENANT: (tenantId: number) =>
        `/api/rentalAgreement-microservice/rental-requests/tenant/${tenantId}`,
      BY_PROPERTY: (propertyId: number) =>
        `/api/rentalAgreement-microservice/rental-requests/property/${propertyId}`,
      UPDATE_STATUS: (requestId: number) =>
        `/api/rentalAgreement-microservice/rental-requests/${requestId}/status`,
      SCORE: (userId: number) =>
        `/api/rentalAgreement-microservice/ai-models/consult-score/${userId}`,
    },
    RENTAL_CONTRACTS: {
      BASE: '/api/rentalAgreement-microservice/rental-contracts',
      USER_ME: '/api/rentalAgreement-microservice/rental-contracts/user/me',
      KEY_DELIVERY: (id: number) =>
        `/api/rentalAgreement-microservice/rental-contracts/${id}/key-delivery`,
    },
    NOTIFICATIONS: {
